export function checkIncorrectNamingConventions(fileContent, filePath) {
    ...
    codeLines.forEach((codeLine, index) => {
        ...
        if (classMatch && !classRegex.test(classMatch[1])) {
            console.log(
                `Class naming violation at line ${index + 1}: ${classMatch[1]}`
            );
            let errorTextLine =
                "Class naming violation at line " +
                (index + 1) +
                ": `" +
                classMatch[1] +
                "`";
        ...
        const varMatch = codeLine.match(
            /\b(?:int|double|String|boolean)\s+(\w+)/
        );
        if (
            varMatch &&
            !variableRegex.test(varMatch[1]) &&
            !constantRegex.test(varMatch[1])
        ) {
            console.log(
                `Variable naming violation at line ${index + 1}: ${varMatch[1]}`
            );
            let errorTextLine =
                "Variable naming violation at line " +
                (index + 1) +
                ": `" +
                varMatch[1] +
                "`";
